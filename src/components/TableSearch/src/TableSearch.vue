...
<template>
  <div class="search-container">
    <el-form :model="model" ref="formRef" size="small">
      <!-- input -->
      <el-form-item v-if="titleItem.searchType?.includes('input')" :prop="titleItem.prop">
        <el-input v-model="model[titleItem.prop]" />
      </el-form-item>
      <!-- radio -->
      <el-form-item v-if="titleItem.searchType?.includes('radio')" :prop="titleItem.prop">
        <el-radio-group v-model="model[titleItem.prop]">
          <el-radio v-for="item in titleItem.radioList" :label="item.value">{{item.label}}</el-radio>
        </el-radio-group>
      </el-form-item>
      <!-- checkbox---尚未完成 -->
      <el-form-item v-if="titleItem.searchType?.includes('checkbox')" :prop="titleItem.prop">
        <el-checkbox-group v-model="model[titleItem.prop]">
          <el-checkbox v-for="item in titleItem.checkboxList" :label="item.label" :name="titleItem.prop" />
        </el-checkbox-group>
      </el-form-item>
      <!-- time -->
      <el-form-item v-if="titleItem.searchType?.includes('time')" :prop="titleItem.prop">
        <el-date-picker v-model="model[titleItem.prop]" type="datetimerange" format="YYYY-MM-DD HH:mm:ss"
